...
/// mean of (a', b') for both conditions; S4.4 consumes the averages for
/// Rg. J' is a PyTM30 extension -- S4.3 specifies only a' and b'.
struct BinAverages {
  std::array<double, 16> J_prime; // Average J' per bin  // PyTM30 extension
  std::array<double, 16> a_prime; // Average a' per bin  // TM-30-20 S4.3
  std::array<double, 16> b_prime; // Average b' per bin  // TM-30-20 S4.3
};
...
struct LocalBinMetrics {
  std::array<double, 16> Rf_hj; // Local fidelity per bin   // TM-30-20 S4.8
  // Local chroma shift per bin, as a PERCENTAGE. Eq. (62) computes a
  // ratio; S4.6 requires percentage representation (Table E-1 range is
  // roughly -100% to 100%). The x100 is applied exactly once, at struct
  // fill in compute_local_bin_metrics. TM-30-20 S4.6
  std::array<double, 16> Rcs_hj_percent;
  // Local hue shift per bin, dimensionless RATIO. S4.7 states no
  // percentage requirement; Table E-1 range is roughly -1 to 1 and the
  // Annex D templates print bare decimals. TM-30-20 S4.7
  std::array<double, 16> Rhs_hj;
  std::array<double, 16> DE_hj; // Mean dE' per bin        // TM-30-20 S4.8
};
...
struct CvgCoordinates {
  /// Test CES bin-averaged J'a'b', normalized per CVG S4.5.
  std::array<double, 16> J_test; // J' unchanged               // TM-30-20 S4.5
  std::array<double, 16>
      x_test; // CVG x (horizontal axis)    // TM-30-20 S4.5 Eq. (60)
  std::array<double, 16>
      y_test; // CVG y (vertical axis)      // TM-30-20 S4.5 Eq. (61)

  /// Reference CES bin-averaged J'a'b', normalized per CVG S4.5.
  std::array<double, 16> J_ref; // J' unchanged               // TM-30-20 S4.5
  std::array<double, 16>
      x_ref; // Reference circle x (cos)   // TM-30-20 S4.5 Eq. (58)
  std::array<double, 16>
      y_ref; // Reference circle y (sin)   // TM-30-20 S4.5 Eq. (59)
};
...
/// TM-30-20 S4.3-S4.8
struct GamutResult {
  double Rg;             // Gamut area index         // TM-30-20 S4.4 Eq. (57)
  BinAverages test_avg;  // Test bin averages       // TM-30-20 S4.3
  BinAverages ref_avg;   // Reference bin averages  // TM-30-20 S4.3
  LocalBinMetrics local; // Per-bin local metrics   // TM-30-20 S4.6-S4.8
  CvgCoordinates cvg;    // CVG coordinates         // TM-30-20 S4.5
};
...
///
/// TM-30-20 S4.3 (closing paragraph); S4.4 consumes the averages for Rg.
