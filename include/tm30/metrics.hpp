...
/// TM-30-20 S4.1
struct RfResult {
  double Rf_prime; // Raw fidelity before log rescale  // TM-30-20 S4.1 Eq. (53)
  double Rf;       // Final fidelity (0 to 100)        // TM-30-20 S4.1 Eq. (54)
  double delta_e_avg; // Average color difference         // TM-30-20 S4.1
};
...
///
/// Steps:
///   1. dE_avg = mean of 99 dE' values                    // TM-30-20 S4.1
///   2. Rf' = 100 - 6.73 * dE_avg                         // TM-30-20 S4.1 Eq.
///   (53)
///   3. Rf = 10 * ln(exp(Rf' / 10) + 1)                   // TM-30-20 S4.1 Eq.
///   (54)
///
/// @param delta_e_array  Array of 99 dE' values.
/// @return               RfResult with Rf_prime, Rf, and delta_e_avg.
///
/// TM-30-20 S4.1 Eq. (53), (54)
RfResult compute_rf(const std::array<double, 99> &delta_e_array);

/// Compute per-sample color fidelity Rf,CESi for each CES.
///
/// For each CES i:
///   Rf,CESi' = 100 - 6.73 * dE'_i              // TM-30-20 S4.2 Eq. (55)
///   Rf,CESi  = 10 * ln(exp(Rf,CESi' / 10) + 1) // TM-30-20 S4.2 Eq. (56)
///
/// TM-30-20 S4.2
std::array<double, 99>
...
///
/// PyTM30 research extension informed by TM-30-20 S4.2 (the CES15/CES18
/// mean correlates with the broader skin-sample set). Not one of the
/// standard's 50 measures (S1.2); per S4.0 not identified as part of
/// the TM-30 method.
