...
///
/// Divides the a'-b' plane into 16 equal hue-angle bins of 22.5° each,
/// starting at 0° (positive a' axis). Each CES is assigned to a bin
...
///
/// Bin j (0-indexed, j = 0…15) spans [(j × 22.5°), ((j+1) × 22.5°)) in
/// the a'-b' plane. Bin 15 spans [337.5°, 360.0°] inclusive.
///
...
///   hr = atan2(b'r,i, a'r,i)    // in radians, normalized to [0, 2π)
///   bin = floor(hr / (π/8))     // 22.5° = π/8 rad per bin
///   Clamp to 15 to handle hr → 2π (≡ 0°)
///
...
///   Bin 16 (0-indexed: 15) spans [337.5°, 360.0°] inclusive.
///
