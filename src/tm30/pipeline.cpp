...
CesColorimetryResult compute_ces_colorimetry(
  ...
  // TM-30-20 §3.1: CIE 1964 10° observer for tristimulus integration
  // TM-30-20 §3.3: CIE 1931 2° observer for CCT determination
  const CmfData cmf2 = resample_cmf(spd_wavelengths, cmf_2deg);
  const CmfData cmf10 = resample_cmf(spd_wavelengths, cmf_10deg);
  // TM-30-20 §3.5

  // ── Step 3: Compute 2° XYZ → CCT / Duv ────────────────────────────────
  // TM-30-20 §3.3: CCT determined from CIE 1931 2° XYZ via Ohno 2014 method
  const SourceXyz src_2deg = compute_source_xyz(
      spd_wavelengths, spd_values, cmf2.x_bar, cmf2.y_bar, cmf2.z_bar);
  // TM-30-20 §3.2 Eq. (1)-(4) with 2° observer
  ...
  // ── Step 5: Compute test source 10° XYZ → normalisation constant kt ───
  ...
  // TM-30-20 §3.2 Eq. (1)-(4) with 10° observer
  ...
  // ── Step 7: Compute reference source 10° XYZ → normalisation kr ───────
  ...
  // TM-30-20 §3.7.1: Adapting to test source white point (10° XYZ)
  {
  ...
  // TM-30-20 §3.7.1: Adapting to reference illuminant white point (10° XYZ)
  {
  ...
  // ── Step 11: Compute ΔE′ and Rf ───────────────────────────────────────
  // TM-30-20 §3.8 Eq. (52): ΔE′ for each CES
